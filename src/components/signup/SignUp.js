...
export default function SignUp() {
  ...
  useEffect(() => {
    ...
    } else if (user.sentEmailVerification && !user.emailIsVerified) {
  ...
  const signUpHandler = () => {
    if (!formValid) return;
    dispatch(actions.signUpUser(emailValue, pswValue));
    //if (!api.error) navigate("/login?msg=signedup");
  };
        ...
        <div className="w-full mt-16 md:mt-0 md:w-2/5">
          <div className="relative z-10 h-auto p-8 py-10 overflow-hidden bg-white border-b-2 border-gray-300 shadow-2xl px-7 rounded-md">
            <h3 className="mb-6 text-2xl font-medium text-center">
              {!api.error && !api.loading && `Sign Up`}
              {api.loading && `Signing up...`}
              {api.error && `${errorParser(api.errorMsg)}`}
              {/* <p>Sentemail: {`${user.sentEmailVerification}`}</p>
              <p>Isverified: {`${user.emailIsVerified}`}</p>
              <p>Id: {`${user.id}`}</p>
